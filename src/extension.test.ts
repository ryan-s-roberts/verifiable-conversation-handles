...
import { ConversationHandleError, parseCallToolHandleError } from './errors.js';
...
describe('mint ordering', () => {
  ...
  it('does not advance latestSeq when encodeHandle throws after a successful mint', async () => {
    ...
    const failed = await append('more', handle);
    expect(failed).toMatchObject({ isError: true });
    expect(parseCallToolHandleError(failed)?.data.reason).toBe('handle_too_large');
    expect(conversationStore.get(cid)?.latestSeq).toBe(1);
  });

  /** §8: mint failures return normative tool errors, not transport throws. */
  it('returns tool error when mint CAS retries are exhausted', async () => {
    const manager = conversationHandlePlugin({
      keys: TEST_KEYS,
      resolvePrincipal: () => 'alice',
      stateCommitment: (record) => encodeMemoryHead(record.memory),
    ...
    const first = await append('seed');
    const handle = handleFromMeta(first._meta);
    vi.spyOn(conversationStore, 'compareAndBumpSeq').mockReturnValue(undefined);

    const failed = await append('more', handle);
    expect(failed).toMatchObject({ isError: true });
    expect(parseCallToolHandleError(failed)?.data.reason).toBe('handle_invalid');
  });
});
